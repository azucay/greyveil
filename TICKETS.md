# TICKETS.md — Ticket-Index

| ID | Titel | Phase | Status |
|---|---|---|---|
| T001 | Projekt Setup | Phase 1 | 🔲 Open |
| T002 | Tile-Map Grundstruktur | Phase 1 | 🔲 Open |
| T003 | Ressourcen-System | Phase 1 | 🔲 Open |
| T004 | Arbeiter-Einheit | Phase 1 | 🔲 Open |
| T005 | Gebäude-System | Phase 1 | 🔲 Open |
| T006 | Einheiten-Kampfsystem | Phase 2 | 🔲 Open |
| T007 | Regelbasierte Ork-KI | Phase 2 | 🔲 Open |
| T008 | Claude API Makrostrategie | Phase 2 | 🔲 Open |
| T009 | Sieg/Niederlage-Bedingung | Phase 2 | 🔲 Open |
| T010 | PWA Setup | Phase 3 | 🔲 Open |
| T021 | KI-Verbesserung — Mehrphasige Strategie | Phase 2 | 🔲 Open |
| T014 | Minimap | Polish/MVP | ✅ Done |
| T022 | Einheiten visuell unterscheidbar machen | Polish/MVP | ✅ Done |
| T023 | Gebäude-Labels besser lesbar machen | Polish/MVP | ✅ Done |
| T024 | Feindliche Gebäude-Schadenszustände sichtbar machen | Polish/MVP | ✅ Done |
| T025 | Gebäude-Labels visuell aufwerten | Polish/MVP | ✅ Done |
| T026 | Ressourcenanzeige auf einheitliche Symbole umstellen | Polish/MVP | ✅ Done |
| T027 | Baufortschritt direkt über Gebäude anzeigen | Polish/MVP | ✅ Done |
| T028 | Gebäudeplatzierung mit Preview und HUD-Bestätigung | Polish/MVP | ✅ Done |
| ERI-1 | Doppelte Baufortschrittsanzeige bei Gebäuden entfernen | Polish/MVP | ✅ Done |
| T029 | Wachturm als defensives Gebäude hinzufügen | Phase 2 | ✅ Done |
| ERI-8 | KI-Basisbau diversifizieren und Angriffswellen wiederherstellen | Phase 2 | ✅ Done |
| T030 | Visual Polish, Waldabbau, Reparaturen und Spielgefühl | Polish/MVP | ✅ Done |
| T031 | Warcraft-2-artige 2D-Asset-Pipeline | Visuals/Post-MVP | 🔄 In Progress |

---

## Status-Legende

| Symbol | Bedeutung |
|---|---|
| 🔲 Open | Noch nicht begonnen |
| 🔄 In Progress | Aktiv in Bearbeitung |
| ✅ Done | Abgeschlossen und gemergt |
| 🚫 Blocked | Blockiert durch Abhängigkeit |

---

## Phasen

**Phase 1 — Foundation (T001–T005)**
Projekt-Grundstruktur, spielbare Karte, Ressourcen sammeln, Gebäude bauen.
Ziel: Ein Spieler kann eine Basis aufbauen und Ressourcen sammeln.

**Phase 2 — Gameplay Loop (T006–T009)**
Kampfsystem, AI-Gegner, Sieg/Niederlage. Vollständiger Spielablauf von Start bis Sieg oder Niederlage.

**Phase 3 — Polish & Distribution (T010)**
PWA-Fähigkeit für Mobile Browser.
